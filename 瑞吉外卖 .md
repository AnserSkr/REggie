...
- **阿里云短信服务：(了解)**
    ...
    **阿里云  短信服务(Short Message Service) 即 SMS。**是广大企业客户快速触达手机用户所优选使用的通信能力。调用API或用群发助手，即可发送验证码、通知类和营销类短信;国内验证短信秒级触达，到达率最高可达99%；国际/港澳台短信覆盖200多个国家和地区，安全稳定，广受出海企业选用。
    
    ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2054.png)
    
    **注册账号：**阿里云官网: [https://www.aliyun.com/](https://www.aliyun.com/)       点击官网首页注册按钮。
    
    **短信服务的使用：**
    
    登录后，在阿里云界面搜索产品—》**短信服务，短信服务需要提前购买。**
    
    购买完成后，**进入控制台—》搜索短信服务，进入短信服务的操作区域，如下**
    
    ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2055.png)
    
    途中已经提示给了我们需要进行一系列操作。**阿里云当前已关闭个人签名服务的支持。个人无法申请**
    
    **申请签名：点击国内消息即可添加，**短信签名是短信发送者的署名，表示发送方的身份。也就是短信最上方的发送来源【阿里云】
    
    ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2056.png)
    
    - **模板管理：切换页面为模板管理，**短信模板包含短信发送内容、场景、变量信息。
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2057.png)
        
        点击详情查看模板信息，**可以手动申请，但是都是在有签名的前提下**
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2058.png)
        
        - 设置AccessKey：光标移动到用户头像上，在弹出的窗口中点击【AccessKey管理】
            
            ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2059.png)
            
        - **注意：打开之后会弹出一个页面，半包含两个按钮。继续使用AccessKey，开始使用子用户的AccessKey。**
            
            ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2060.png)
            
            **继续使用AccessKey：此方式获取的Key具有最高权限，一旦泄露等同于泄露账户。不建议使用**
            
            **开始使用子用户的AccessKey：此方式会创建一个新的AccessKey，这个Key是受限的，比较安全一些。**
            
        - **使用子用户的Key需要先创建一个子用户：确定后输入验证码即可创建**
            
            ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2061.png)
            
            创建完成后进入用户界面，进行权限授予，点击蓝色的名称，进入用户管理页面—》权限管理
            
            然后在下方搜索SMS短信服务，将权限授予即可
            
            ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2062.png)
            
            **授权成功之后，就可以在项目中调用API进行短信服务了**
            
        
        如果AccessKey泄露，就可以进入到用户界面的认证管理，直接禁用这个Key，该Key就失效了
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2063.png)
        
- **短信发送编码—测试版**
    
    **使用阿里云短信服务发送短信，可以参照官方提供的文档即可，不需要手动的进行编码。一般都可以查看帮助文档解决。帮助文档移到右上角用户头像左边的问号那里了**
    
    **使用步骤：导入Maven坐标—》调用API**
...
- **手机验证码登录**
    - **需求分析**
        ...
        **数据表：使用的user表，**该表主要就是保存的前台的互联网用户
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2064.png)
        
        由于涉及到User表的操作，因此创建User实体，UserMapper，UserService，UserServiceIpml，UserController
        
        用户登录的页面**(front/page/login.html)**输入手机号，点击【获取验证码】按钮，页面发送ajax请求，在服务端调用短信服务API给指定手机号发送验证码短信
        
        —》在登录页面输入验证码，点击【登录】按钮，发送ajax请求，在服务端处理登录请求
        
        开发手机验证码登录功能，其实就是在服务端编写代码去处理前端页面发送的这2次请求即可。
        
        首先对于前端界面当点击获取验证码时，按钮会自动调用**`getCode`**方法，生成验证码。
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2065.png)
        
        前端页面需要请求`login`和`sendmsg`，因此`LoginCheckFilter`如果没有放行则不能够完成操作，因此需要先将路径进行放行。
        
        **随后，即可进行编码**
        
        **首先**修改前端页面的`**getCode**`方法，并调用`**sendMsgApi()`发送Post请求/user/sendMsg**用于向后端传递发送验证码请求。同时验证码的生成与校验工作交给后端完成，不在前端生成随机验证码。**完成后端编码。**
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2066.png)
        
        发送到后端，首先判断phone是否存在，如果不存在根据phone新建用户。随后生成随机码，用于进行验证码发送。
        
        **随后**就是登陆操作，发送POST请求到/user/login页面`{phone: "15189654627", code: "1234"}`，后端进行验证码和手机号的校验。
        
        **登陆成功后则会跳转到`/front/index.html`页面**
        
    - **代码编写**
        - **首先是发送验证码，请求地址`/user/sendMsg`，请求内容phone:15236498745**
            
            **UserController：**
            
            ```java
            @PostMapping("/sendMsg")
            public Result<String> sendMsg(HttpServletRequest request, @RequestBody String phone) throws Exception {
                //首先根据phone是否是新用户
        ...
        - **登录请求，请求地址/user/login，请求内容**`{phone: "15189654627", code: "1234"}`
            
            前端发送请求后，后端使用map接收，如果登陆成功，则返回成功状态
...
- **前端分析**
    ...
    用户登录成功后会进入到商店和菜品的展示页面，再此页面当资源加载完毕之后会调用mounted钩子函数，执行操作，此处调用的是`initData()`方法
    
    此方法内部一次性调用了两个方法用于发送AJAX请求，`**Promise.all([*categoryListApi*(),*cartListApi*({})]).then(res=>{`，并且要求是连个请求都请求成功才能加载出来所有内容，否则就按失败处理.**
    
    *`categoryListApi*()`方法发送的是GET请求**/category/list**获取所有的分类数据。*`cartListApi*({})`方法页发送GET请求用于请求**/shoppingCart/list.**
    
    - **此处有一个调用的小技巧，直接让cartListApi拿取我们设定好的json数据，假装获取到了后台的数据，进行响应处理。用于临时代替后端的操作：**
        
        ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2067.png)
        
    
    同时当两个方法都执行完毕并获取到有意义的返回值之后，就会自动的调用`getDishList()` 发送Get请求到**/dish/list 用于根据categoryID获取对应的菜品数据。并存放到`dishList`中。**
    
    **分类下菜品展示**
    
    以上方式**从`dishList`查询出来dish数据，但是并不能进行口味选择，选择口味则需要修改返回值为dishDto。如下**
    
    想要的效果是，当所有菜品信息展示出来之后，每当点击每一个菜品时，会调用`dishDetails(item)`将该行dish的信息封装到item中传递过去。然后弹出一个详细信息的对话框。
    
    在对话框展示数据中我们可以查看到套餐对应的flavor口味信息，因此在调用**`gitDishList()`时获取到的Dish不仅要包含dish基础信息，还需要包含flavor信息。**
    
    改造DIshController下请求**/dish/list**的返回值为DIshDto，交给前端展示flavors信息
    
    初始状态时会自动展示位于第一个未知的套餐分类的信息。
    
    当点击不同的分类时，会自动的调用`**categoryClick(index,item.id,item.type**)`将该分类的各项信息传递过去。在方法内根据type的不同，调用`**getDishList()`发送GET**请求**/dish/list**或者调用`getSetmealData()`请求**/setmeal/list**
    
    **套餐展示：**
    
    由于type为2，则会调用`getSetmealData()`请求**/setmeal/list?categoryId=1413342269393674242&status=1，返回套餐分类下的所有套餐信息**
    
    如果想要查看详细的套餐信息，点击套餐时，会调用`dishDetails**(item)**`方法，将选中行的套餐信息传递进来。根据有无口味信息判断是菜品还是套餐。
    
    如果没有口味信息flavors则为套餐，调用*`setMealDishDetailsApi***(item.id)` 发送GET请求/setmeal/dish/${id}用于获取setmealId对应的全部菜品。**查询的数据只需要setmeal对应的setmealDish数组即可
    
    最后将查询到的**setmeal对应的菜品数组保存到前端js对象对应的该行数据item的list中**，用于遍历展示。
    
    由于前端还需要展示图片信息，因此需要封装一个setmealDIshDto对象，用于保存各个DIsh的image的路径位置
...
- **需求分析**
    ...
    地址簿，保存移动端消费者用户的地址信息，用户登录成功后可以维护自己的地址信息。同一个用户可以有多个地址信息，但是只能有一个**默认地址**。
    
    ![Untitled](%E7%91%9E%E5%90%89%E5%A4%96%E5%8D%96%20fcee46ea152c4c49b3455bd197b7c5db/Untitled%2068.png)
    
    点击用户图标后，会调用`toUserPage()`，跳转到用户的个人信息页`/front/page/user.html。`
    
    **地址薄的展示**
    
    点击用户信息页的地址管理后，会调用`toAddressPage()`，跳转到用户的地址管理页`/front/page/address.html`
    
    随后会调用钩子函数*`addressListApi()` 请求*`/addressBook/list`。获取当前登录用户的所有的`**AddressBook**`对象存放于`**addressList`，**用于展示地址信息。
    
    此处需要用到Address_Book数据库表，我们引入AddressBook的类，AddressBookMapper，AddressBookService，AddressBookServiceImpl以及AddressBookController
    
    编辑后端代码
    
    **地址薄的新增**
    
    当点击**添加收货地址**后会调用`toAddressCreatePage()`，跳转到`/front/page/address-edit.html`页面。
    
    进入该页面，**会自动调用钩子函数，判断是否携带有address的id值。以确定当前操作为**新增address还是修改address。
    
    信息输入页面内所有的信息都放置在js对象**`form`**中，当点击保存地址之后，会判断是否有id值，没有时发送POST请求，进行新增操作*`addAddressApi(**from**)`*
    ...
    **设置默认地址：**
    ...
    点击设置默认地址前的勾选，会调用`setDefaultAddress**(item)**`方法，将当前行所在的addressBook封装到`**item**`中传递过来。
    
    若item的id不为空，则调用*`setDefaultAddressApi(***id:item.id***)*`函数，向后端发送PUT请求`/addressBook/default`设定当前**addressBook**为用户的默认地址。
    
    编辑后端编码
    
    **对已有地址进行编辑：**
    
    点击已有地址的编辑按钮后，会发送**/front/page/address-edit.html?id=${item.id}**请求，而**address-edit.html页面的初始化方法会根据有无id数据，调用***`addressFindOneApi***(params.id)**`方法向后端发送GET请求`/addressBook/${id}`用于获取当前编辑的addressbook数据。
    
    修改后，点击保存地址，会调用`saveAddress()`方法，在方法内部根据有无addressBook的id数据来选择调用更新或者新增，此处有id数据调用*`**updateAddressApi*(this.form)**`
    
    将封装有修改后addresBook的数据封装到**data**中传递到发送Put请求的**/addressBook，操作成功后自动跳转到地址展示列表**
    
    编辑后端编码
    
    **在编辑页面删除已有地址：**
    
    进入页面后会自动的请求到修改的addressBook数据，点击删除按钮后会调用`deleteAddress()`方法，当确认删除后会请求*`deleteAddressApi*(**{ids:this.id }**)`将当前addressVook的id数据封装到ids中，向后端发送delete请求**/addressBook，用于删除指定ID的addressBook，操作成功后会自动地跳转到地址展示页面。**
...
- **前端分析**
    ...
    首先需要明白的逻辑是，菜品加入购物车的流程。
    
    - **菜品以及套餐的购物车添加**
        
        首先点击菜品的选择口味，当点击添加规格后，会自动调用`chooseFlavorClick(**item**)`将代表当前菜品的DishDto封住到item中传递到后端，在方法中将数据封装到**dilogFlavors**中。展示时是前端使用了for循环，直接对**dishFlavors**的口味数组进行展示。
        
        当点击对应的口味之后，会调用`flavorClick(flavor,item)`将当前口味对应的flavor对象以及选中的内容封装到**item**中传递过去，**同时将选中的口味信息添加到flavors.dishFlavor中**，然后在方法内强制刷新口味信息的弹窗，再次将选中的dishFlavors信息展示的选中状态展示出来。**此处的数据都保存到dialogFlavors.flavors.dishFlavors中**
        
        **当点击加入购物车后**，会调用`dialogFlavorAddCart()`方法，内部会遍历**保存在dialogFlavors.flavors.dishFlavors中的选中的菜品口味。如果对应的flavor对象有选中的口味则会保存到新创建的dishFlavors中，如果没有选中口味的话，会自动提示要选择口味(每个口味都需要选择，否则提示)。**
        
        **当将选中的口味信息都提取出来后调用**`addCart(**data**)`方法。将保存有dish信息的**dishFlavor**其中的price，dishID,image,name以及存储选中口味dishFlavor数组以`“,”`间隔转换为字符串存入dihsflavor。整体封装为date数据传递过去。
        
        在`addCart(**data**)`方法中，将数据提取出来封装到**prams**中，根据有无口味信息判断是菜品还是套餐，并赋值对应的ID。然后调用*`addCartApi*(params)`发送POST请求到**/shoppingCart/add，发送的Json对象如下**
        
        ```json
        name: "邵阳猪血丸子",image: "2a50628e-7758-4c51-9fbb-d37c61cdacad.jpg",
        dishId: "1397851370462687234",,dishFlavor: "少冰,不要香菜,重辣",
        amount: 138
        ```
        
        由函数的返回后操作可以得知，加入购物车成功，需要将shoppingCart对象返回，前端数据需要根据**res.data.number**更新前端数量信息
        
        最后，调用`getCartData()`方法再次请求**/shoppingCart/list到**后端获取到购物车保存的所有菜品并将其返回,并将购物车数据保存到**cateDate**中。
        
    - **菜品的删除以及购物车清空**
        
        **菜品上的删除：**
        
        当点击菜品上的**`+`**或者`**-`时，会自动的调用**`addCart**(item)**`**和**`subtractCart**(item)**`**方法，**
        
        调用`**subtractCart(item)`方法时，会将操作的行的shoppingCart数据封装到item中传递过去。在方法内部判断DishId和SetmealID哪一个存在，然后封装到params中**
        
        然后调用*`updateCartApi***(params**)` 方法，发送POST请求到后端**/shoppingCart/sub，内部只封装dishId或setmealId**
        
        **购物车中的删除：**
        
        当点击购物车的的**`+`**或者`**-`时，会自动的调用`cartNumAdd(item)`****和**`cartNumberSubtract**(item)**`**方法，而`cartNumAdd()`方法与addCart方法几乎一致**
        
        调用`**cartNumberSubtract(item)`方法时，会将操作的行的shoppingCart数据封装到item中传递过去。在方法内部直接将item中的dishId和setmealId封装到params中，**
        
        然后调用*`updateCartApi***(params**)` 方法，发送POST请求到后端**/shoppingCart/sub**
        
        **删除和添加后需要将shoppingcart的信息返回到前端，前端的展示界面需要res.data.number的值用于展示**
        
        **购物车的清空：**
        
        点击清空按钮后，会调用`clearCart()`方法发送**delete**请求到**/shoppingCart/clean**
        
        在后端成功清空后，自动将前端的所有内容置空。同时关闭购物车弹窗
...
- **前端分析**
    - **支付前数据准备**
        ...
        点击去结算按钮后会自动的调用`toAddOrderPage()`方法，跳转页面到**/front/page/add-order.html。在该页面的初始化钩子函数`created()`会调用**`initData()`方法，
        
        分别调用`defaultAddress()`以及`getCartData()`向后端发送请求。用于获取地址信息和购物车保存的菜品信息。
        
        在`defaultAddress()`方法内首先调用*`getDefaultAddressApi*()`向后端发送**GET**请求**/addressBook/default**获取默认地址信息。
        
        如果default响应吗为1，则对返回数据进行处理，由前端所需要的响应信息得知，该请求需要返回默认地址信息代表的addressBook对象，用于数据展示。然后调用`getFinishTime()`方法获取送达的时间。
        
        如果响应码为0，表示没有默认地址，则自动跳转到**/front/page/address-edit.html**'进行Address的添加添加成功后则会跳转到address展示页，用于展示address，选中默认地址后返回继续完成下单操作
        
        在`getCartData()`方法内，使用*`cartListApi`*方法向后端发送Get请求**/shoppingCart/list**获取到所有的购物车菜品信息
        
        此时你可能有一个问题？总金额是怎么计算的？再下单页面，有连个模型数据`**goodsNum`和`goodsPrice`两个模型数据保存了单个菜品的总金额以及所有菜品的总数量。参数的计算**是通过Vue的`**computed()`计算器**完成的，当所有的数据加载完毕后，会自动的调用computed中参数对应的方法计算两个参数。
        
    
    后端操作时首先需要创建Order对象，OrderMapper，OrderService，OrderServiceImpl，以及OrderController以及OrderDetail对象，OrderDetailMapper，OrderDetailService,OrderDetailServiceImpl,OrderDetailController
    
    当点击了**去支付**操作后，会调用`goToPaySuccess()`方法，将添加的备注note，支付方式，以及下单的addressId封装到**params**中。
    
    然后请求*`addOrderApi***(params)`，**发送**Post**请求到**/order/submit**将**params**提交到后端。params数据为`addressBookId: "1576794888119992321"，payMethod: 1，remark: "士大夫士大夫士大夫"，amount:306`
    
    后端需要将传递过来的数据封住到Order对象中，然后根据UserId查询出购物车信息，将购物车中的每一个dishId对应的dishFlavor保存到orderDetail中，所有操作都成功之后，返回一个订单存放成功的返回信息
...
- **前端代码分析**
    ...
    下单成功后，原本需要进行付款，此处并未引入支付接口，所以并不会出现付款界面，会直接展示一个下单成功过的界面，我们点击查看订单后，会自动跳转到**/front/page/order.html**页面，此页面具有created的初始函数
    
    在钩子函数中调用`**GetList()**`方法，在方法内部会调用 `orderPagingApi(page,pageSize)`向后端发送Get的请求到**/order/userPage，将返回值存储到`orderList`中，并在之后对此数据进行循环遍历。**
    
    每一个订单orders对象中含有一个**orderDetails**数组，对**OrderDetails**数组进行遍历，将每一个菜品的数量相加，作为总数量赋值给**sumNum**
    
    基于以上分析可得，返回值为Page对象，且records中的对象为**OrderDto**，内部包含一个OrderDetail数组
...
**前端代码分析：**当在用户界面点退出登录后，会调用`**toPageLogin()`，**同时调用*`loginoutApi()`*发送POST请求到**/user/loginout**
...
首先在个人信息页面点击再来一单，会调用 `*orderAgainApi*(**{id:this.order[0].id}**)`并将第一个订单的id传递过去

而在订单信息页也会调用*`orderAgainApi*(**{id:order.id}**)`将当前选中行的Orderid传递过去，用户生成新的购物车内容

***`orderAgainApi*(data)`方法会发送POST请求到/order/again**
...
后端点击订单明细后跳转到**/backend/page/order/list.html，**此页面有一个初始函数`**created()**`自动调用**`init()`**方法。

init方法内会调用*`getOrderDetailPage*({ **page: this.page, pageSize: this.pageSize**, **number**: this.input || undefined, **beginTime:** this.beginTime || undefined, **endTime:** this.endTime || undefined })`，发送**get请求到/order/page。用户进行数据的查询**
...
当点击派送后会调用`**cancelOrDeliveryOrComplete(3, row.id)`，会先提示我们是否确认，点击确认后会调用*`editOrderDetail*(params)`将status和orderId传递到后端。**发送请求的方式为**PUT**请求地址为**/order**

完成订单操作同理。

MYpatisPlus的更新操作会自动屏蔽掉null值，因此更新Order即可
